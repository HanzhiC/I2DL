...
def relu_backward(dout, cache):
    ...
    #############################################################################
    d_local = x.copy()
    d_local = np.where(d_local>0, 1, 0)
    dx = d_local * dout
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def batchnorm_backward(dout, cache):
    ...
    dx_norm = dout * gamma
    dvar = np.sum(-0.5 * dx_norm * x_minus_mean * ivar**3, axis = 0 )
    dmean = (1/N) * dvar * np.sum(-2 * x_minus_mean, axis = 0) + np.sum (-1 * dx_norm * ivar, axis = 0)
    dx = dx_norm * ivar + (2/N) * dvar * x_minus_mean + (1/N) * dmean
